  ...
  function renderizarFormProduto() {
    return (
      <div className="flex gap-5 items-end">
        ...
        <div>
          {produto.id ? (
            <button onClick={alterarProduto} className="bg-blue-500 px-4 py-2 rounded-md">Alterar produto</button>
          ) : (
            <button onClick={criarProduto} className="bg-blue-500 px-4 py-2 rounded-md">Criar produto</button>
          )}
        </div>
      </div>
    )
    ...
    return (
      <div className="flex flex-col gap-2">
        {produtos.map((produto: any) => (
          <div key={produto.id} className="flex gap-2 bg-zinc-800 px-4 py-2 rounded-md items-center">
